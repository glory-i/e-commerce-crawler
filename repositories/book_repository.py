...
async def get_all_book_urls(db: AsyncIOMotorDatabase) -> Set[str]:
    """
    Get set of all book URLs in database
    ...
    try:
        cursor = db.books.find({}, {'source_url': 1, '_id': 0})
        urls = {doc['source_url'] async for doc in cursor}
        logger.info(f"Found {len(urls)} books in database")
        return urls
    except Exception as e:
        logger.error(f"Error getting book URLs: {e}")
        return set()
    

async def count_books(db: AsyncIOMotorDatabase) -> int:
    """
